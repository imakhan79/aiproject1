...
import { Lightbulb, Mic, MicOff, Send, ChevronDown, ChevronUp, Trash2 } from 'lucide-react';
...
function ProjectRoadmap({ dream }: { dream: Dream }) {
  return (
    <div className="space-y-6">
      <p className="text-slate-300 leading-relaxed">{dream.structuredPlan.overview}</p>
      ...
      <div>
        <h4 className="font-semibold text-white mb-3">Project Phases</h4>
        <div className="space-y-3">
          {dream.structuredPlan.phases.map((phase, i) => (
            <div key={phase.name} className="glass rounded-xl p-4">
              <div className="flex items-center gap-3 mb-3">
                <span className="w-7 h-7 rounded-full bg-purple/30 border border-purple/50 text-purple-glow text-xs font-bold flex items-center justify-center">{i + 1}</span>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-10">
                <div>
                  ...
                  <ul className="space-y-1">
                    {phase.tasks.map((t) => <li key={t} className="text-xs text-slate-400 flex gap-2"><span className="text-cyan mt-0.5">✓</span>{t}</li>)}
                  </ul>
                </div>
                <div>
                  <div className="text-xs text-slate-500 mb-1.5">Resources</div>
                  <ul className="space-y-1">
                    {phase.resources.map((r) => <li key={r} className="text-xs text-slate-400 flex gap-2"><span className="text-purple-glow mt-0.5">→</span>{r}</li>)}
                  </ul>
                </div>
              </div>
            </div>
      ...
      <div>
        <h4 className="font-semibold text-white mb-3">12-Week Learning Roadmap</h4>
        <div className="space-y-2">
          {dream.learningRoadmap.milestones.map((m) => (
            <div key={m.week} className="flex gap-4 items-start">
              <div className="flex-shrink-0 w-16 text-xs text-slate-500 mt-0.5">Week {m.week}</div>
...
function DreamCard({ summary, onExpand }: { summary: DreamSummary; onExpand: (id: string) => void }) {
  return (
    <button onClick={() => onExpand(summary.id)}
      className="glass glass-hover rounded-2xl p-5 text-left w-full transition-all">
...
export default function DreamsPage() {
  ...
  const { data: list } = useQuery<DreamSummary[]>({
    queryKey: ['dreams'],
    queryFn: async () => { const { data } = await api.get('/dreams'); return data; },
  });

  const { data: dreamDetail } = useQuery<Dream>({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold gradient-text">Dream Capture</h1>
        <p className="text-slate-400 mt-1">Speak or write any idea — AI turns it into a project plan</p>
      </div>
      ...
      <div className="glass rounded-2xl p-6">
        ...
        <div className="flex justify-between items-center">
          <button onClick={toggleVoice}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm border transition-all ${listening ? 'bg-red-500/20 border-red-500/40 text-red-400' : 'glass border-white/10 text-slate-300 hover:text-white'}`}>
            {listening ? <><MicOff size={16} /> Stop Recording</> : <><Mic size={16} /> Voice Input</>}
          </button>
          <button onClick={() => create.mutate()} disabled={!rawInput.trim() || create.isPending}
            className="flex items-center gap-2 px-5 py-2 rounded-xl bg-gradient-to-r from-purple to-cyan text-white font-medium hover:opacity-90 disabled:opacity-40 text-sm">
            <Send size={16} />
      ...
      {expanded && dreamDetail && (
        <div className="glass rounded-2xl p-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-white">{dreamDetail.title}</h2>
              <p className="text-xs text-slate-500 mt-1">{formatDate(dreamDetail.createdAt)}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setExpanded(null)} className="p-2 glass rounded-xl text-slate-400 hover:text-white">
                <ChevronUp size={18} />
              </button>
              <button onClick={() => deleteDream.mutate(expanded)} className="p-2 glass rounded-xl text-slate-400 hover:text-red-400">
                <Trash2 size={18} />
              </button>
            </div>
          </div>
      ...
      {list && list.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-white mb-4">
            <Lightbulb size={20} className="inline mr-2 text-cyan" />
            Your Dreams ({list.length})
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
