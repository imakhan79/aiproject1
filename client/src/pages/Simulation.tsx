  ...
  return (
    <div className="glass rounded-2xl p-6 space-y-4 flex flex-col">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-xs text-cyan uppercase tracking-wider mb-1">{s.pathName}</div>
          <h3 className="text-lg font-semibold text-white">{s.jobTitle}</h3>
        </div>
        <div className="text-right flex-shrink-0">
          <div className={`text-2xl font-bold ${probColor}`}>{s.probability}%</div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="glass rounded-xl p-3">
          <div className="flex items-center gap-1 text-xs text-slate-500 mb-1"><TrendingUp size={12} /> Salary</div>
          <div className="text-sm font-medium text-white">{formatSalary(s.salaryRange.min, s.salaryRange.max)}</div>
        </div>
        <div className="glass rounded-xl p-3">
          <div className="flex items-center gap-1 text-xs text-slate-500 mb-1"><Clock size={12} /> Timeline</div>
          <div className="text-sm font-medium text-white">{s.timelineYears} years</div>
        </div>
      </div>
      ...
      {s.topCompanies?.length > 0 && (
        <div>
          <div className="flex items-center gap-1 text-xs text-slate-500 mb-2"><Building2 size={12} /> Top companies</div>
          ...
          <ul className="space-y-1">
            {s.keyMilestones.map((m) => (
              <li key={m} className="text-xs text-slate-400 flex items-start gap-2">
                <span className="text-purple-glow mt-0.5">→</span>{m}
              </li>
            ))}
          </ul>
        </div>
...
export default function SimulationPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold gradient-text">Future Simulation Engine</h1>
        <p className="text-slate-400 mt-1">Select 2–3 skill paths to simulate your career futures</p>
      </div>

      <div className="glass rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-white">Choose Skill Paths ({selected.length}/3)</h2>
          {selected.length >= 2 && (
            <button onClick={() => run.mutate()} disabled={run.isPending}
              className="flex items-center gap-2 px-5 py-2 rounded-xl bg-gradient-to-r from-purple to-cyan text-white font-medium hover:opacity-90 disabled:opacity-50 text-sm">
              <Zap size={16} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
          {SKILL_PATHS.map((p) => (
            <button key={p} onClick={() => toggle(p)}
              className={`p-3 rounded-xl text-sm border transition-all text-left ${selected.includes(p) ? 'bg-purple/20 border-purple/50 text-purple-glow' : 'bg-white/5 border-white/10 text-slate-300 hover:border-white/20'} ${!selected.includes(p) && selected.length === 3 ? 'opacity-40 cursor-not-allowed' : ''}`}>
              {p}
            </button>
          ))}
        </div>
      ...
      {current && (
        <div>
          ...
          <div className={`grid gap-4 ${current.results.length === 2 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-3'}`}>
      ...
      {!current && history && history.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-white mb-4">Past Simulations</h2>
