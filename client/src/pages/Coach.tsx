...
export default function CoachPage() {
  ...
  const { messages, setMessages, streaming, streamingText, error, sendMessage } = useChat(history ?? []);
  ...
  const SUGGESTIONS = ["What's my biggest strength?", "What should I learn first?", "How do I start a startup?"];

  return (
    <div className="max-w-3xl mx-auto flex flex-col" style={{ height: 'calc(100dvh - 120px)' }}>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold gradient-text">AI Life Coach</h1>
          <p className="text-slate-400 mt-0.5 text-xs md:text-sm">Powered by your Future Twin profile</p>
        </div>
        {messages.length > 0 && (
      ...
      <div className="flex-1 overflow-y-auto space-y-3 mb-3 chat-scroll pr-1 min-h-0">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4 text-center py-8">
            ...
            <p className="text-slate-500 max-w-xs text-sm">Ask me anything about your career, learning strategy, or next steps. I know your full profile.</p>
        ...
        {streaming && streamingText && (
          <div className="flex justify-start">
            <div className="max-w-[85%] md:max-w-[75%] glass border border-white/10 text-slate-200 rounded-2xl px-4 py-3 text-sm leading-relaxed">
              {streamingText}
        ...
        {error && (
          <div className="flex justify-center">
            <div className="flex items-center gap-2 px-4 py-2 bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl text-sm">
      ...
      <div className="flex gap-2 md:gap-3 flex-shrink-0">
        ...
        <button onClick={handleSend} disabled={!input.trim() || streaming}
          className="px-3 md:px-4 rounded-2xl bg-gradient-to-r from-purple to-cyan text-white hover:opacity-90 disabled:opacity-40 transition-opacity flex-shrink-0">
          <Send size={18} />
        </button>
      </div>
    </div>
